      ...
      <div className="w-72 shrink-0 space-y-6">
          ...
          <div>
            ...
            <div className="space-y-1.5">
              {solvedToday.map(p => {
                ...
                return (
                  <div key={p.id} className={`border rounded-lg px-3 py-2 flex items-center justify-between ${solved ? 'bg-green-200 border-green-300' : 'bg-red-200 border-red-300'}`}>
                    <span className={`text-sm font-medium truncate ${solved ? 'text-green-900' : 'text-red-900'}`}>
                      {p.leetcodeNumber ? `${p.leetcodeNumber}. ` : ''}{p.title}
                    </span>
                    <span className={`text-xs font-semibold ml-2 shrink-0 ${solved ? 'text-green-700' : 'text-red-700'}`}>
        ...
        {upcoming.length > 0 && (
          <div>
            ...
            <div className="space-y-1.5">
              {upcoming.map(p => (
                <div key={p.id} className="bg-gray-50 rounded-lg border border-gray-100 px-3 py-2">
                  ...
                  <div className="flex items-center justify-between mt-1">
                        ...
                        return (
                          <span
                            ...
                            className={`inline-block w-1.5 h-1.5 rounded-full ${
                              isDone ? 'bg-indigo-500' : isCurrent ? 'bg-indigo-300' : 'bg-gray-200'
                            }`}
                          />
                        )
                      })}
                    ...
                    <div className="text-right">
                      <div className="text-xs text-gray-400">{formatUpcomingDate(p.nextReviewDate)}</div>
                      <div className="text-xs text-gray-300">{p.nextReviewDate}</div>
                    </div>
                  </div>
                </div>
              ))}
